...
def test_should_work_when_directory_exists(create_file: callable) -> None:
    os.makedirs("dir")
    move_file(f"mv file.txt dir/file1.txt")

    with open("dir/file1.txt", "r") as file_with_content:
        assert file_with_content.read() == "This is some\n content for\n the file."
...
def test_should_create_multiple_directories(create_file: callable) -> None:
    move_file(f"mv file.txt first_dir/second_dir/file1.txt")

    assert os.path.exists("first_dir/second_dir/file1.txt") is True
    assert os.path.exists("file.txt") is False

    with open("first_dir/second_dir/file1.txt", "r") as file_with_content:
        assert file_with_content.read() == "This is some\n content for\n the file."
...
def test_should_create_multiple_directories_when_they_exist(create_file: callable) -> None:
    ...
    move_file(f"mv file.txt first_dir/second_dir/third_dir/file1.txt")

    with open("first_dir/second_dir/third_dir/file1.txt", "r") as file_with_content:
        assert file_with_content.read() == "This is some\n content for\n the file."
