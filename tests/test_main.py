...
def test_should_work_when_directory_exists(create_file: callable) -> None:
    os.makedirs("dir")
    move_file(f"mv file.txt dir/text.txt")

    with open("dir/text.txt", "r") as file_with_content:
        assert file_with_content.read() == "This is some\n content for\n the file."
...
def test_should_create_multiple_directories(create_file: callable) -> None:
    move_file(f"mv file.txt first_dir/second_dir/text.txt")

    assert os.path.exists("first_dir/second_dir/text.txt") is True
    assert os.path.exists("file.txt") is False

    with open("first_dir/second_dir/text.txt", "r") as file_with_content:
        assert file_with_content.read() == "This is some\n content for\n the file."
...
def test_should_create_multiple_directories_when_they_exist(create_file: callable) -> None:
    ...
    move_file(f"mv file.txt first_dir/second_dir/third_dir/text.txt")

    with open("first_dir/second_dir/third_dir/text.txt", "r") as file_with_content:
        assert file_with_content.read() == "This is some\n content for\n the file."
