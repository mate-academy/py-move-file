...
def move_file(command: str) -> None:
    ...
    if len(parts) != 3 or parts[0] != "mv":
        raise ValueError("Command must be in format: mv source_file destination_path")

    src = parts[1]
    dst = parts[2]

    # Якщо кінцевий шлях закінчується на '/', вважаємо його директорією і беремо ім'я оригінального файлу
    if dst.endswith("/"):
